...
const EnvSchema = z.object({
    ...
    INVITE_DEFAULT_USAGE_COUNT_MODE: z.enum(["unique_guest", "every_claim"]).default("unique_guest"),
    INVITE_ONLY_GUEST_ENABLED: z.coerce.boolean().default(false),
    GUEST_SESSION_TTL_HOURS: z.coerce.number().int().positive().default(24),
    GUEST_MEMBER_RETENTION_DAYS: z.coerce.number().int().positive().default(30),
    INVITE_AUDIT_RETENTION_DAYS: z.coerce.number().int().positive().default(180),
    GUEST_CLEANUP_INTERVAL_MINUTES: z.coerce.number().int().positive().default(30),
